...
const ModalChangePassword = ({ setShowModalChangePassword }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await notifier.asyncBlock(
      axios.patch(
        `https://nocountrybackend.onrender.com/api/v1/users/${sessionStorage.getItem(
          "userId"
        )}/changePassword`,
        data,
        { headers }
      ...
      (err) => {
        console.log(err);
        notifier.alert(
          `Ocurrió un error al cambiar la contraseña, intentelo de nuevo más tarde`
        );
        setShowModalChangePassword(false);
      },
  ...
  return (
    <>
      <div className="h-screen w-screen fixed top-0 left-0 z-[100] flex justify-center items-center bg-[rgba(1,1,1,0.6)]">
              ...
              >
                <div className="flex flex-col align-bottom w-[80%] text-lg">
                  ...
                  <input
                    ...
                    name="oldPassword"
                    className="bg-dark border-yellow border-b-2 focus:outline-0"
                  />
                </div>
                <div className="flex flex-col align-bottom w-[80%] text-lg">
                  <label
